...
export default async function JobsPage() {
  ...
  const [{ data: leads }, { data: snaggingLeads }, { data: archivedLeads }] = await Promise.all([
    ...
    supabase
      .from('leads')
      .select('id, name, address, postcode, stage, job_date, job_end_date, snagging_signed_off_at, tasks(title, notes, type, completed_at)')
      .eq('stage', 'snagging')
  ...
  type SnaggingRow = { id: string; name: string; address: string | null; postcode: string | null; job_date: string | null; job_end_date: string | null; tasks: { title: string; notes: string | null; type: string; completed_at: string | null }[] }
