...
export function LeadsTable({ leads, total, page, pageSize, duplicateEmails }: Props) {
  ...
  function exportCSV() {
    ...
    const rows = [
      ['Name', 'Mobile', 'Email', 'Address', 'City', 'Postcode', 'Stage', 'Source'],
      // eslint-disable-next-line @typescript-eslint/no-explicit-any
      ...selectedLeads.map(l => [l.name, l.mobile ?? '', l.email ?? '', l.address ?? '', (l as any).city ?? '', l.postcode ?? '', l.stage ?? '', l.lead_source ?? '']),
    ]
        ...
        <div className="hidden md:block overflow-x-auto">
          <table className="w-full text-sm">
            ...
            <tbody className="divide-y divide-gray-50">
              {leads.length === 0 ? (
                <tr>
                  <td colSpan={11} className="px-4 py-10 text-center text-gray-400 text-sm">No leads found</td>
                </tr>
              ) : leads.map(lead => {
